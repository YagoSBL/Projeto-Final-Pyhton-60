...
def inserir_lead():
    ...
    # Código da biblioteca que permite retornar o status do lead 
...
# Read 
def mostrar_leads(): 
    # Aqui serão exibidos os leads na treeview
    ...
    leads = c.fetchall()
   # Todos os dados serão pegos e transmitidos à treeview
    for lead in leads:
...
def preencher_campos(event):
    ...
    if selecao:
        ...
        # Aqui está definido o valor que será atribuído ao combobox (widget de interface gráfica) 

# Atualização de dados do lead selecionado
def atualizar_lead():
    selecao = tree.selection()
    if selecao:
        ...
        if novo_nome and novo_email and novo_telefone and novo_interesse and novo_status:
            ...
            c = conn.cursor()
            try:
                # O email original é usado no WHERE para atualizar o registro
                c.execute("""
                    UPDATE leads 
                ...
                messagebox.showerror('Erro', 'O novo E-mail já está cadastrado para outro Lead')
            finally:
                conn.close()
        else:
            messagebox.showwarning('Atenção', 'Todos os campos precisam ser preenchidos para a atualização')
    else:
...
def deletar_lead():
    ...
    if selecao:
        ...
        if messagebox.askyesno('Aviso', 'Tem certeza que deseja deletar o lead selecionado?'):
            conn = conectar()
            c = conn.cursor()        
            c.execute("DELETE FROM leads WHERE email = ?", (email_del,))
            conn.commit()
            conn.close()
            messagebox.showinfo('Aviso', 'Lead deletado com sucesso!')
            limpar_campos()
            mostrar_leads()
    else:
...
janela = tk.Tk()
janela.title('CRM de Leads - Registro de Leads Yasbel')
...
# Título
tk.Label(janela, text='📈 Formulário de cadastro de leads', font=('Arial', 18, 'bold')).grid(row=0, column=0, pady=10, padx=10, columnspan=2)
...
# Status do Lead - O combobox está sendo usado
...
# Frame para os botões do CRUD
fr = tk.Frame(janela)
fr.grid(row=2, column=0, columnspan=2, pady=15,)

# Botões
btn_salvar = customtkinter.CTkButton(fr, text='➕ SALVAR NOVO', font=('Arial', 12, 'bold'), fg_color= 'green',  command=inserir_lead)
btn_salvar.grid(row=0, column=0, padx=10, pady=5, ipadx=5)

btn_atualizar = customtkinter.CTkButton(fr, text='✏️ ATUALIZAR STATUS/DADOS', font=('Arial', 12, 'bold'), fg_color= 'blue', command=atualizar_lead)
btn_atualizar.grid(row=0, column=1, padx=10, pady=5, ipadx=5)

btn_delete = customtkinter.CTkButton(fr, text='🗑️ DELETAR LEAD', font=('Arial', 12, 'bold'), fg_color= 'red',  command=deletar_lead)
...
# Vinculação da seleção da Treeview à função que preenche os campos
